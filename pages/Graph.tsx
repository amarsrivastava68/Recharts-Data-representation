...
const Graph = ({ jsonData, theme }: { jsonData: any[], theme: string }) => {
  ...
  const yAxisOptions = ['Air Temperature', 'Dew Point Temperature', 'Cloud Height', 'Visibility', 'Wind Direction'];
  ...
  return (
    <div className="flex justify-between">
      ...
      <div className="flex flex-col items-start mr-4 ml-5">
        <p className={`mb-2 font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>Select Y-axis Parameter:</p>
        {yAxisOptions.map((option, index) => (
          <button
            ...
            className={`mb-2 py-2 px-4 rounded-lg text-base  transition duration-300 ease-in-out ${
              theme === 'dark'
                ? `text-white ${yAxisDataKey === option ? 'bg-blue-600' : 'bg-gray-800 hover:bg-gray-700'}`
      ...
      <div className="flex flex-col items-end mr-5">
        <p className={`mb-2 font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>Select X-axis Parameter:</p>
        {xAxisOptions.map((option, index) => (
          <button
            ...
            className={`mb-2 py-2 px-4 rounded-lg text-base  transition duration-300 ease-in-out ${
              theme === 'dark'
                ? `text-white ${xAxisDataKey === option ? 'bg-green-600' : 'bg-gray-800 hover:bg-gray-700'}`
